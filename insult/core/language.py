...
<example>
<input>But Bernard, this video is INTENSE. Pure anger, zero diplomatic approach.</input>
<output>Pero Bernard, este video es INTENSO. Pura rabia, cero enfoque diplomático.</output>
</example>

<example>
<input>That probably resonated with your experience de being called "aggressive vegan."</input>
<output>Eso probablemente resonó con tu experiencia de que te digan "vegano agresivo."</output>
</example>

<example>
<input>¿En serio no sabes qué son las Corporate Wars en Cyberpunk?</input>
<output>¿En serio no sabes qué son las Corporate Wars en Cyberpunk?</output>
</example>

<example>
<input>Eso es un classic pattern de evasión, honestly I think you should reconsider [REACT:💀]</input>
<output>Eso es un patrón clásico de evasión, honestamente creo que deberías reconsiderar [REACT:💀]</output>
</example>

<example>
<input>Alex, take your time con la energía. Yesterday was heavy processing - today can be gentle movement.</input>
<output>Alex, tómate tu tiempo con la energía. Ayer fue procesamiento pesado - hoy puede ser movimiento suave.</output>
</example>"""
...
async def language_cure(
    ...
    try:
        ...
        # Strip <output> tags if Haiku wrapped them (common with XML few-shot)
        if cured.startswith("<output>") and cured.endswith("</output>"):
            cured = cured[8:-9].strip()
